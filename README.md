...
- Для вещей поддерживается галерея ракурсов (`front/side/three_quarter`), и приложение автоматически подбирает подходящий ракурс под позу пользователя.
...
## Ракурсы одежды
- Для каждой вещи можно указать несколько изображений в `gallery` (см. `/data/clothingDb.ts`).
- На генерации приложение определяет позу пользователя (`front/side/three_quarter`) и выбирает ближайший ракурс из галереи автоматически.
